...
#### Throughout the summer and fall of 2020, water samples were collected from the Whitefish Bay, Wisconsin water distribution system. Water quality parameters were measured and the following ranges of values were obtained.
...
paste("The average decrease in Chlorine residual from one station to the next was," , round(mean((for_analyte_boxplots %>%
                                                                                                       filter(analyte %in% "Chlorine_Free_mg_L") %>% mutate(Station_Num = as.numeric(as.character((Station_Num)))) %>% group_by(sample_name_abridged) %>% slice(1) %>% ungroup() %>% filter(Collection_Date != "2020-10-01") %>% group_by(Collection_Date) %>% arrange(-(Station_Num)) %>% mutate(dif = value - lag(value)) %>% filter(!is.na(dif)) %>% ungroup() %>% group_by(Station_Num) %>% summarize(mean_decr = mean(dif)))$mean_decr), 2), "ppm")

chlorine_scatter_plot
```

#### Note: Samples take on 10/01/20 were clearly anamolous. The chlorine residual increased distally from the treatment plant. This was later determined to be because of an issue in the treatment plant, which was adding too much chlorine into the finished water the night prior to sampling.
